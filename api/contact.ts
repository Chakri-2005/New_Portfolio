...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CORS setup if accessed directly (Vercel automatically sets this up generally but good practice)
  ...
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    ...
    res.status(500).json({ error: 'Email service not configured on the server. Please contact me directly.' });
    return;
  }
